...
- 📊 Metrics and dashboards: provider ranking, success rate trends, request history.
- 🛡️ Built-in auth, API key issuance, role/permission, and security middleware.
...
- 📊 指标与看板：Provider 排行、成功率趋势、请求历史。
- 🛡️ 一站式安全：鉴权、API Key 发行、角色/权限、中间件安全校验。
- 🧰 开发体验：FastAPI 后端 + Next.js 管理台，docker-compose 一键本地栈。
